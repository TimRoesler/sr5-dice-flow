...
- ✅ **Heilung** (`heal`-FlowSpec): matcht `action.skill` ∈ {first_aid, biotechnology, medicine}; Ziel-Branch(es) = Patient(en); „Heilung anwenden"-Button (`actor.healDamage('physical', origin.hits)`, verifizierte API) — advisorisch, nur per Bestätigung.
